...
export const parseCompoundMessage = async (messageText, username) => {
  // Check if message might be compound by looking for common patterns
  const lines = messageText.split('\n').map(line => line.trim()).filter(line => line.length > 0);
  
  // If only one line, not compound
  if (lines.length <= 1) {
    return { isCompound: false };
  }
  
  console.log('Checking for compound message with lines:', lines);
  
  // Try to parse each line separately to see if we have completion + start
  let completionResult = null;
  let startResult = null;
  
  for (const line of lines) {
    const parsed = await parseMessage(line, username);
    
    if (parsed.type === 'task_completion' && !completionResult) {
      completionResult = parsed;
      console.log('Found completion in compound message:', line);
    } else if (parsed.type === 'task_start' && !startResult) {
      startResult = parsed;
      console.log('Found task start in compound message:', line);
    }
  }
  
  // If we found both completion and start, it's compound
  if (completionResult && startResult) {
    console.log('Detected compound message: completion + start');
    return {
      isCompound: true,

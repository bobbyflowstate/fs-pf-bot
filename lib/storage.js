...
import { categorizeTasksBatch } from './parser.js';
...
export const getUserSummary = async (chatId, userId) => {
  try {
    ...
    // Categorize tasks from last 24 hours
    let categoryBreakdown = [];
    try {
      const taskDescriptions = recentCompletedTasks
        .map(task => task.task_description)
        .filter(desc => desc && desc.trim().length > 0);
      
      if (taskDescriptions.length > 0) {
        console.log('Categorizing', taskDescriptions.length, 'tasks from last 24h');
        const categories = await categorizeTasksBatch(taskDescriptions);
        
        // Group tasks by category and calculate time per category
        const categoryTimes = {};
        recentCompletedTasks.forEach((task, index) => {
          if (task.task_description && task.task_description.trim().length > 0) {
            const category = categories[taskDescriptions.indexOf(task.task_description)] || 'Other';
            categoryTimes[category] = (categoryTimes[category] || 0) + (task.actual_minutes || 0);
          }
        });
        
        // Convert to array and sort by time spent
        categoryBreakdown = Object.entries(categoryTimes)
    ...
    // Get last 4 tasks for recent activity
    const recentTasks = completedTasks.slice(0, 4);
